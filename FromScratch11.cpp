	...
	for (int x = 0; x < imgRotatedAndScaled.cols; x++) {
		...
		for (int y = 0; y < imgRotatedAndScaled.rows; y++) {
			...
			if (matPixelWasSet.at<uchar>(y, x) == 0) {
				...
				int intCountOfPixelsAroundWithinBorder = 0;
					...
					for (int innerY = 0; innerY < intWindowSize; innerY++) {
						...
						if ((x + innerX - intOffset) >= 0 && (x + innerX - intOffset + 2) <= imgRotatedAndScaled.cols &&
							...
							if (imgRotatedAndScaled.at<uchar>(y + innerY - intOffset, x + innerX - intOffset) > 0) {
								...
								intCountOfPixelsAroundWithinBorder++;
							}
						}
					}
					//std::cout << "out of 4" << std::endl;
				}
				//std::cout << "out of 3" << std::endl;
				if (intCountOfPixelsAroundWithinBorder > 0) {
					imgRotatedAndScaled.at<uchar>(y, x) = intTotalOfPixelsAround / intCountOfPixelsAroundWithinBorder;
				}
				...
				int uchAverageOf8Around = 0;
				//int intCountOfPixelsAroundWithinBorder = 0;

				if ((x - 1) >= 0 && (y - 1) >= 0) {													// left 1, up 1
					if (imgRotatedAndScaled.at<uchar>(y - 1, x - 1) > 0) {
						uchAverageOf8Around = uchAverageOf8Around + imgRotatedAndScaled.at<uchar>(y - 1, x - 1);
						intCountOfPixelsAroundWithinBorder++;
					}
				}

				if ((x - 2) >= 0 && (y - 1) >= 0) {													// left 2, up 1
					if (imgRotatedAndScaled.at<uchar>(y - 1, x - 2) > 0) {
						uchAverageOf8Around = uchAverageOf8Around + imgRotatedAndScaled.at<uchar>(y - 1, x - 2);
						intCountOfPixelsAroundWithinBorder++;
					}
				}

				if ((x - 1) >= 0 && (y - 2) >= 0) {													// left 1, up 2
					if (imgRotatedAndScaled.at<uchar>(y - 2, x - 1) > 0) {
						uchAverageOf8Around = uchAverageOf8Around + imgRotatedAndScaled.at<uchar>(y - 2, x - 1);
						intCountOfPixelsAroundWithinBorder++;
					}
				}

				if ((x - 2) >= 0 && (y - 2) >= 0) {													// left 2, up 2
					if (imgRotatedAndScaled.at<uchar>(y - 2, x - 2) > 0) {
						uchAverageOf8Around = uchAverageOf8Around + imgRotatedAndScaled.at<uchar>(y - 2, x - 2);
						intCountOfPixelsAroundWithinBorder++;
					}
				}
				...
				if ((y - 1) >= 0) {																	// up 1
					if (imgRotatedAndScaled.at<uchar>(y - 1, x) > 0) {
						uchAverageOf8Around = uchAverageOf8Around + imgRotatedAndScaled.at<uchar>(y - 1, x);
						intCountOfPixelsAroundWithinBorder++;
					}
				}

				if ((y - 2) >= 0) {																	// up 2
					if (imgRotatedAndScaled.at<uchar>(y - 2, x) > 0) {
						uchAverageOf8Around = uchAverageOf8Around + imgRotatedAndScaled.at<uchar>(y - 2, x);
						intCountOfPixelsAroundWithinBorder++;
					}
				}
				...
				if ((x + 1) < imgRotatedAndScaled.cols && (y - 2) >= 0) {							// right 1, up 2
					if (imgRotatedAndScaled.at<uchar>(y - 2, x + 1) > 0) {
						uchAverageOf8Around = uchAverageOf8Around + imgRotatedAndScaled.at<uchar>(y - 2, x + 1);
						intCountOfPixelsAroundWithinBorder++;
					}
				...
				if ((x + 2) < imgRotatedAndScaled.cols && (y - 1) >= 0) {							// right 2, up 1
					if (imgRotatedAndScaled.at<uchar>(y - 1, x + 2) > 0) {
						uchAverageOf8Around = uchAverageOf8Around + imgRotatedAndScaled.at<uchar>(y - 1, x + 2);
						intCountOfPixelsAroundWithinBorder++;
					}
				...
				if ((x + 2) < imgRotatedAndScaled.cols && (y - 2) >= 0) {							// right 2, up 2
					if (imgRotatedAndScaled.at<uchar>(y - 2, x + 2) > 0) {
						uchAverageOf8Around = uchAverageOf8Around + imgRotatedAndScaled.at<uchar>(y - 2, x + 2);
						intCountOfPixelsAroundWithinBorder++;
					}
				...
				if ((x + 1) < imgRotatedAndScaled.cols) {											// right 1
					if (imgRotatedAndScaled.at<uchar>(y, x + 1) > 0) {
						uchAverageOf8Around = uchAverageOf8Around + imgRotatedAndScaled.at<uchar>(y, x + 1);
						intCountOfPixelsAroundWithinBorder++;
					}
				}

				if ((x + 2) < imgRotatedAndScaled.cols) {											// right 2
					if (imgRotatedAndScaled.at<uchar>(y, x + 2) > 0) {
						uchAverageOf8Around = uchAverageOf8Around + imgRotatedAndScaled.at<uchar>(y, x + 2);
						intCountOfPixelsAroundWithinBorder++;
					}
				}
				...
				if ((x + 2) < imgRotatedAndScaled.cols && (y + 1) < imgRotatedAndScaled.rows) {		// right 2, down 1
					if (imgRotatedAndScaled.at<uchar>(y + 1, x + 2) > 0) {
						uchAverageOf8Around = uchAverageOf8Around + imgRotatedAndScaled.at<uchar>(y + 1, x + 2);
						intCountOfPixelsAroundWithinBorder++;
					}
				...
				if ((x + 1) < imgRotatedAndScaled.cols && (y + 2) < imgRotatedAndScaled.rows) {		// right 1, down 2
					if (imgRotatedAndScaled.at<uchar>(y + 2, x + 1) > 0) {
						uchAverageOf8Around = uchAverageOf8Around + imgRotatedAndScaled.at<uchar>(y + 2, x + 1);
						intCountOfPixelsAroundWithinBorder++;
					}
				...
				if ((x + 2) < imgRotatedAndScaled.cols && (y + 2) < imgRotatedAndScaled.rows) {		// right 2, down 2
					if (imgRotatedAndScaled.at<uchar>(y + 2, x + 2) > 0) {
						uchAverageOf8Around = uchAverageOf8Around + imgRotatedAndScaled.at<uchar>(y + 2, x + 2);
						intCountOfPixelsAroundWithinBorder++;
					}
				...
				if ((y + 2) < imgRotatedAndScaled.rows) {											// down 2
					if (imgRotatedAndScaled.at<uchar>(y + 2, x) > 0) {
						uchAverageOf8Around = uchAverageOf8Around + imgRotatedAndScaled.at<uchar>(y + 2, x);
						intCountOfPixelsAroundWithinBorder++;
					}
				...
				if ((x - 1) >= 0 && (y + 2) < imgRotatedAndScaled.rows) {							// left 1, down 2
					if (imgRotatedAndScaled.at<uchar>(y + 2, x - 1) > 0) {
						uchAverageOf8Around = uchAverageOf8Around + imgRotatedAndScaled.at<uchar>(y + 2, x - 1);
						intCountOfPixelsAroundWithinBorder++;
					}
				...
				if ((x - 2) >= 0 && (y + 1) < imgRotatedAndScaled.rows) {							// left 2, down 1
					if (imgRotatedAndScaled.at<uchar>(y + 1, x - 2) > 0) {
						uchAverageOf8Around = uchAverageOf8Around + imgRotatedAndScaled.at<uchar>(y + 1, x - 2);
						intCountOfPixelsAroundWithinBorder++;
					}
				...
				if ((x - 2) >= 0 && (y + 2) < imgRotatedAndScaled.rows) {							// left 2, down 2
					if (imgRotatedAndScaled.at<uchar>(y + 2, x - 2) > 0) {
						uchAverageOf8Around = uchAverageOf8Around + imgRotatedAndScaled.at<uchar>(y + 2, x - 2);
						intCountOfPixelsAroundWithinBorder++;
					}
				...
				imgRotatedAndScaled.at<uchar>(y, x) = uchAverageOf8Around / intCountOfPixelsAroundWithinBorder;
				*/
			}
